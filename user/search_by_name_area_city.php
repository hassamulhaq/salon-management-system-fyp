...
$salon_area_location = $_GET['salon_area_location'];
$salon_city = $_GET['salon_city'];

if (!empty($_GET['salon_name'])) {
    $sql = "SELECT * FROM tbl_salon WHERE name LIKE '%$salon_name%'";
} elseif (!empty($_GET['salon_area_location'])) {
    $sql = "SELECT * FROM tbl_salon WHERE address LIKE '%$salon_area_location%'";
} else {
    $sql = "SELECT * FROM tbl_salon WHERE city LIKE '%$salon_city%'";
}
...
if ($stmt->rowCount() > 0) {
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) { ?>
        <form action="appointment_create_step2.php" method="post">
